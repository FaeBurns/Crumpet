...
{
    public Result<object> RunFile(FileInfo file, string entryPointName, string[] args, Stream? inputStream, Stream? outputStream)
    {
        try
        {
            ...
            Result<Variable> result = executor.StepUntilComplete();

            if (result.IsFailure)
                return Result<object>.Error(result.ErrorMessage!);

            return Result<object>.Ok(result.Value!.GetValue()!);
        }
        catch (Exception e)
        {
